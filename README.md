...
- The plugin stores the full email payload in an option keyed by ses_mailer_job_{uuid} (autoload = no) and schedules a tiny cron argument containing only { job_id }.
...
- PHPUnit test suite under tests/ with WordPress function stubs.
